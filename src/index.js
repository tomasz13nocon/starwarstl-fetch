...
const docFromTitle = async (title) => {
  let page = (await fetchWookiee(title, true).next()).value;
  if (page.missing)
    return null;
...
if (debug.saveTimeline) {
  ...
  log.info("Saving timeline wikitext to ./timeline due to debug.saveTimeline");
}
...
for (let [i, item] of data.entries()) {
  ...
  // draft.altTitle = decode(item.Title.text).split("*")[0].replaceAll(/''+|†/g, "").trim();
  draft.altTitle = decode(item.Title.text).split("*")[0].replace("†", "").trim().replace(/^"(.*)"$/, "$1");
...
if (debug.distinctInfoboxes) {
  await writeFile("infoboxes.txt", infoboxes);
