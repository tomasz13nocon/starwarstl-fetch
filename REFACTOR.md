...
After each step, besides marking steps as complete, if needed, write a completion note for future agents. Include only information a future agent genuinely needs and cannot infer from repository state, branch contents, git history, or existing markdown docs: hidden context, decisions, caveats, or user intent that would otherwise be lost. Do not include current branch/status facts, or general process notes. Some steps will not need a note. This is an addition - not a replacement - to documenting code you write, and writing any other .md files as needed. Ask yourself if this step and conversation with the user revealed something, that future agents working on following steps will NEED to know. If the answer is no, don't include the note. It's not meant to be a history of "this and that was done", it's a collection of important notes, future agents need to know to adequately complete the next stages of the refactor.
